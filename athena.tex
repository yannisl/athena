...
% Display title
      ...
      \if@titlespaceout
         \so{#2}%
      \else
         \texorpdfstring{#2}%title NEEDS CHECKING
      \fi%
      \titleafter@cx
    \if@righttitle%
      \afternumber@cx
...
%                                   {\protect\numberline{\thechapter}#1}%
              ...
              \addcontentsline{toc}{chapter} {\protect\numberline{%
                   ...
                   \Rule\kern-13cm\hspace{1em}\bfseries\sffamily {#2}}}%\MakeUppercase fix for hyperref
           \fi%
           \else
...
% This is a parameterized version of the default \tableofcontents definition. Normally each class has its own definition, as well as various packages. We try not to break anything.
\@specialfalse
...
\keyval{toc chapter precis}{\marg{true|false}}{This is experimental. If a chapter precis is specified in a chapter, then the precis is typeset in the ToC also\footnote{Such a command is provided by the tocloft package and in the memoir class}.}
%}
...
This is a general description of how \LaTeXe\ processes the Table of Contents. As the processing for a list of fifures and a List of Tables is similar we will only discuss the ToC.

\cs{addcontentsline} LaTeX generates a \file{.toc} if the document contains a \cs{tableofcontents} command and a lot or lof if a caption command is activated. 
...
\section{The \string\tableofcontents}
...
%\end{document}
One can vary the \cs{colorscheme} command to produce as many templates as required.
...
\newgeometry{left=5cm,right=2.5cm, marginparsep=15pt, marginparwidth=4.2cm,top=2cm,%
reversemarginpar}
